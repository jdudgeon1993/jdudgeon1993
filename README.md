...
        <img src="https://media.licdn.com/dms/image/v2/D5603AQGLMLr9wZ1MDA/profile-displayphoto-crop_800_800/B56ZfH.oFBHoAM-/0/1751406772949?e=1766016000&v=beta&t=NyBNEaZtoVxFWjbgDhtwsDlmfDkXULkgsSQHdZPx3io" alt="Jordan Dudgeon Profile Picture"/>
      </div>
...
<script>
    // ----------------------------------
    // PROJECT DATA ARRAY (Project 1 mediaUrl set to live site URL)
    // ----------------------------------
    const projectsData = [
        { 
...
            // Media URL is the live site link for iframe embedding
...
            // Custom button label logic (uses 'View Live Site' for Project 1)
...
            if (project.mediaUrl) {
                // NEW LOGIC: Embed live site in an iframe if mediaUrl matches the link (Project 1)
                if (project.mediaUrl === project.link && project.link.includes("github.io")) {
                    const iframe = document.createElement('iframe');
                    iframe.src = project.mediaUrl;
                    iframe.setAttribute('title', project.title + ' Live View');
...
                    mediaContainer.appendChild(iframe);
                } 
                // Existing image logic (for all other projects)
                else {
