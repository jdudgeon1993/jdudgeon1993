...
<head>
  ...
  <style>
    ...
    .project-modal-content {
      ...
      padding: 30px 10px;
      min-height: 250px;
      position: relative; /* make navigation arrows positioned correctly */
    }
...
<body class="bookish">
  <div class="toggle-theme-btn-container">
    <button class="toggle-btn" id="toggleThemeBtn" aria-label="Switch between Bookish, Dungeon, and Seasonal Themes">Switch Theme</button>
  </div>

  <main>
    <div class="card" role="region" aria-label="Profile card">
      <div class="profile-pic">
        <img src="https://media.licdn.com/dms/image/v2/D5603AQGLMLr9wZ1MDA/profile-displayphoto-crop_800_800/B56ZfH.oFBHoAM-/0/1751406772949?e=1766016000&v=beta&t=NyBNEaZtoVxFWjbgDhtwsDlmfDkXULkgsSQHdZPx3io" alt="Jordan Dudgeon Profile Picture" onerror="this.style.display='none'"/>
      </div>
      ...
      <div class="info">
        ...
        <a href="https://www.linkedin.com/in/jordan-dudgeon" target="_blank" rel="noopener" aria-label="Connect with me on LinkedIn">LinkedIn</a>
        <button id="projectsToggleBtn" onclick="toggleSection('projects')" aria-expanded="false" aria-controls="projects">Projects</button>
    ...
    <div class="projects" id="projects" role="region" aria-labelledby="projects-heading">
      ...
      <div class="project-item" role="button" tabindex="0" onclick="openProjectGallery(0)" onkeydown="if(event.key==='Enter'||event.key===' ') openProjectGallery(0)">
      ...
      <div class="project-item" role="button" tabindex="0" onclick="openProjectGallery(1)" onkeydown="if(event.key==='Enter'||event.key===' ') openProjectGallery(1)">
      ...
      <div class="project-item" role="button" tabindex="0" onclick="openProjectGallery(2)" onkeydown="if(event.key==='Enter'||event.key===' ') openProjectGallery(2)">
  ...
  <div class="modal-backdrop" id="contactModal" onclick="closeModal('contactModal')" aria-hidden="true">
    <div class="modal-content" role="dialog" aria-modal="true" aria-labelledby="contact-title" onclick="event.stopPropagation()">
      ...
      <p><strong>Email:</strong> <a href="mailto:jdudgeon1993@gmail.com">jdudgeon1993@gmail.com</a></p>
  ...
  <div class="modal-backdrop" id="projectModal" onclick="closeModal('projectModal')" aria-hidden="true">
    <div class="modal-content project-modal-content" role="dialog" aria-modal="true" aria-labelledby="project-title" onclick="event.stopPropagation()">
      <button class="modal-close-btn" onclick="closeModal('projectModal')" aria-label="Close Project Gallery">&times;</button>

      <button class="nav-btn left-btn" onclick="navigateProjects(-1)" aria-label="Previous Project"><span class="nav-btn" aria-hidden="true">&lt;</span></button>

      <div class="project-details fade-out">
        <div class="project-media-container" id="project-media"></div>
        <h2 id="project-title"></h2>
        <p id="project-description"></p>
        <a id="project-link" href="#" target="_blank" rel="noopener" class="info-link">View Project</a>
      </div>

      <button class="nav-btn right-btn" onclick="navigateProjects(1)" aria-label="Next Project"><span class="nav-btn" aria-hidden="true">&gt;</span></button>
    </div>
  </div>
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const projectsData = [
      {
        ...
        mediaUrl: "https://jdudgeon1993.github.io/jdudgeon1993/",
    ...
    // Ensure a theme class exists
    if (!themes.some(t => body.classList.contains(t))) {
      body.classList.add('bookish');
    }
    ...
    window.toggleTheme = function toggleTheme() {
      // find current theme safely
      let currentTheme = themes.find(t => body.classList.contains(t));
      const nextIndex = (themes.indexOf(currentTheme) + 1) % themes.length;
      const nextTheme = themes[nextIndex];

      // replace only known theme classes
      themes.forEach(t => body.classList.remove(t));
      body.classList.add(nextTheme);
    };

    // attach to visible button (defensive)
    ...
    function anyModalOpen() {
      ...
      const project = qs('#projectModal');
      return (contact && contact.classList.contains('show')) || (project && project.classList.contains('show'));
    }

    window.openModal = function openModal(id) {
      // Close projects accordion if open
      const projectsAccordion = qs('#projects');
      const projectsBtn = qs('button[aria-controls="projects"]');
      if (projectsAccordion && projectsAccordion.classList.contains('show')) {
        projectsAccordion.classList.remove('show');
        setAriaExpanded(projectsBtn, false);
      }

      // Close project gallery if opening contact
      if (id === 'contactModal') closeModal('projectModal');
    ...
    window.closeModal = function closeModal(id) {
      ...
      // only re-enable scrolling if no modal is open
      if (!anyModalOpen()) {
        document.body.style.overflow = 'auto';
      }
    };

    // Close modals when clicking background handled inline via onclick attributes (they call closeModal)
    ...
    window.toggleSection = function toggleSection(id) {
      ...
      // Close contact modal if opening projects
      closeModal('contactModal');

      const isShowing = section.classList.toggle('show');
      setAriaExpanded(button, isShowing);

      if (isShowing) {
        // ensure focus and visibility for keyboard users
    ...
    /* -------------------------
       ...
       ------------------------- */
    function applyFade(detailsEl, cb) {
      if (!detailsEl) { if (cb) cb(); return; }
    ...
    window.renderProject = function renderProject(index) {
      const detailsContainer = qs('.project-details');
      ...
      applyFade(detailsContainer, () => {
        // Update title/desc/link
        const titleEl = qs('#project-title');
        const descEl = qs('#project-description');
        const linkEl = qs('#project-link');
        const mediaContainer = qs('#project-media');
        ...
        if (linkEl) {
          ...
          // choose label
          if (project.buttonLabel) linkEl.textContent = project.buttonLabel;
          else if (project.link && (project.link.toLowerCase().includes('github') || project.link.toLowerCase().includes('code'))) linkEl.textContent = 'View Code';
          else linkEl.textContent = 'View Project';
        }
        ...
        if (mediaContainer) {
          ...
          if (project.mediaUrl) {
            // Embed live site in iframe if it's same origin-friendly github.io (still depends on headers)
            if (project.mediaUrl === project.link && project.link.includes('github.io')) {
              const iframe = document.createElement('iframe');
              iframe.src = project.mediaUrl;
              iframe.title = project.title + ' Live View';
              iframe.setAttribute('frameborder', '0');
              iframe.setAttribute('sandbox', 'allow-scripts allow-same-origin allow-forms'); // safe-ish sandbox
              iframe.style.width = '100%';
              iframe.style.height = '300px';
              iframe.style.border = '1px solid #ccc';
              mediaContainer.appendChild(iframe);
            } else if (project.mediaUrl.includes('youtube.com') || project.mediaUrl.includes('vimeo.com')) {
              const iframe = document.createElement('iframe');
              iframe.src = project.mediaUrl;
              iframe.setAttribute('frameborder', '0');
    ...
    window.navigateProjects = function navigateProjects(direction) {
      renderProject(currentProjectIndex + direction);
    };

    window.openProjectGallery = function openProjectGallery(startIndex = 0) {
      // Close contact modal while project gallery is open
      closeModal('contactModal');

      // render and show the modal
      renderProject(startIndex);
    ...
    document.addEventListener('keydown', function(event) {
      ...
      if (event.key === 'Escape') {
        if (projectModal && projectModal.classList.contains('show')) closeModal('projectModal');
        if (contactModal && contactModal.classList.contains('show')) closeModal('contactModal');
      }

      if (projectModal && projectModal.classList.contains('show')) {
        if (event.key === 'ArrowRight') navigateProjects(1);
        if (event.key === 'ArrowLeft') navigateProjects(-1);
      }
    });
    ...
    /* -------------------------
       Initial UI hints
       ------------------------- */
    // Ensure project-details has fade-in for initial state (hidden until used)
    const detailsEl = qs('.project-details');
    if (detailsEl) {
      detailsEl.classList.remove('fade-out');
      detailsEl.classList.add('fade-in');
    }

    // Set initial aria-expanded on Projects button
    setAriaExpanded(qs('#projectsToggleBtn'), false);

    // Expose some debug helpers on window for dev convenience (optional)
    window.__projectsData = projectsData;
  });
</script>
</body>
</html>
